...
const App = () => {
  ...
  const setCiudad = async ({ciudad}) => { 
    console.log("hola ciudad",ciudad);
    console.log(process.env);
    const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${ciudad}&units=metric&appid=${process.env.REACT_APP_API_KEY}`);  
